...
class StatCard(ctk.CTkFrame):
    def __init__(self, parent, label: str, value: str = "â€”",
...
class Sims4ModGuardApp(ctk.CTk):
    def __init__(self):
        super().__init__()
        self.title("â—ˆ Sims4 Mod Guardian â—ˆ  by Hucifer & ðŸ¦‰Hypatia")
    ...
    def _build_tabs(self, parent):
        ...
        # Left: tab buttons
        tab_btn_col = ctk.CTkFrame(tab_bar, fg_color=BG_HEADER, width=160)
        ...
        tabs = [
            ("ðŸ”  SCAN SCRIPTS",  "scan",    self._build_scan_tab),
            ("ðŸ“¦  CC CLEANER",    "cc",      self._build_cc_tab),
            ("ðŸ“‹  LOG ANALYZER",  "logs",    self._build_logs_tab),
            ("ðŸ”§  FIX & REPAIR",  "fix",     self._build_fix_tab),
            ("ðŸ’»  CONSOLE",       "console", self._build_console_tab),
            ("â„¹ï¸   ABOUT",         "about",   self._build_about_tab),
        ]
    ...
    def _build_cc_tab(self, parent):
        ...
        NeonButton(action_row, "ðŸ“¦ SCAN CC PACKAGES",
    ...
    def _build_logs_tab(self, parent):
        ...
        NeonButton(action_row, "ðŸ“‹ PARSE LAST EXCEPTION LOG",
    ...
    def _build_fix_tab(self, parent):
        # Big action buttons
        ctk.CTkLabel(parent, text="âš¡ REPAIR CONSOLE âš¡",
        ...
        actions = [
            ("ðŸ”´ QUARANTINE ALL CRITICAL SCRIPTS", NEON_RED,    self._quarantine_critical),
            ("â™»  CLEAR ALL CACHES",               NEON_AMBER,  self._clear_cache),
            ("ðŸŸ¢ RESTORE ALL QUARANTINED",         NEON_GREEN,  self._restore_all),
            ("ðŸ“ OPEN MODS FOLDER",                NEON_CYAN,   self._open_mods_folder),
            ("ðŸ—‘  REMOVE DUPLICATE PACKAGES",      NEON_PINK,   self._remove_duplicates),
            ("ðŸ“‹ SHOW QUARANTINE MANIFEST",        TEXT_DIM,    self._show_manifest),
        ]
...
""",
        ...
        ctk.CTkLabel(parent,
                     text="Community Edition â€” Free Forever",
                     font=("Courier New", 10),
...
WHAT THIS DOES
  ...
  ðŸ”´  Scans .ts4script files for broken injection patterns
  ðŸ”´  Detects APIs removed by EA in patch 1.121
  ðŸŸ¡  Finds WickedWhims dependencies without WW installed
  ðŸŸ¡  Validates every CC package DBPF header
  ðŸŸ¡  Finds duplicate packages by name and content hash
  ðŸ”µ  Parses lastException.txt into plain English
  ðŸŸ¢  Safe quarantine with full restore support
  ðŸŸ¢  One-click cache clearing
...
""",
    ...
    def _build_statusbar(self):
        ...
        self._status_var = tk.StringVar(value="â—ˆ READY â€” Select a Sims 4 folder to begin")
    ...
    def _auto_detect(self):
        ...
        if folder:
            ...
            self._status("â—ˆ Could not auto-detect Sims 4 folder â€” click BROWSE")
    ...
    def _set_folder(self, folder: Path):
        ...
        self._status(f"â—ˆ Sims 4 folder loaded â€” {scripts} scripts, {packages:,} packages")
    ...
    def _boot_sequence(self):
        msgs = [
            ...
            ("by Hucifer & ðŸ¦‰Hypatia â€” Community Edition", "pink"),
            ("â”€" * 55, "dim"),
    ...
    def _run_script_scan(self):
        if not self.mods_folder:
            ...
            return
        self._log("â”€" * 55, "dim")
        self._log("âš¡ Starting script scan...", "header")
        self._scan_progress.set(0)
    ...
    def _finish_scan(self, results):
        ...
        self._switch_tab("scan")
        self._status(f"â—ˆ Scan done â€” {len(critical)} critical, {len(clean)} clean")
    ...
    def _run_cc_scan(self):
        if not self.mods_folder:
            ...
            return
        self._log("â”€" * 55, "dim")
        self._log("ðŸ“¦ Starting CC package scan...", "header")
    ...
    def _finish_cc(self, data):
        ...
        self._switch_tab("cc")
        self._status(f"â—ˆ CC scan done â€” {s['corrupt']} corrupt, {s['duplicate_names']} dup names")
    ...
    def _show_log_summary(self, summary):
        ...
        if not summary.tuning_finished:
            t.append("âš   TUNING DID NOT FINISH LOADING â€” this caused the crash!", "critical")
        ...
        self._status(f"â—ˆ Log parsed â€” {summary.total_errors} errors, "
    ...
    def _quarantine_critical(self):
        ...
        self._status(f"â—ˆ {moved} scripts quarantined â€” clearing cache")

    def _clear_cache(self):
        if not self.s4_folder:
            return
        self._fix_log("â™» Clearing Sims 4 caches...", "info")
